...
function renderUserPage(session){
  ...
  session.groups.forEach(group=>{
    ...
    card.className = 'group-card ' + (full? 'status-full' : (warn? 'status-warn':'' ));
    const header = document.createElement('div');
    header.className = 'card-header';
    header.innerHTML = `<h3>${esc(group.name)}</h3><div class="badge">${group.members.length} / ${group.capacity}</div>`;
    const list = document.createElement('ul');
    list.innerHTML = group.members.map(m=>`<li>${esc(m.name)}</li>`).join('');
...
function renderAdminPage(session){
  ...
  session.groups.forEach(group=>{
    ...
    header.innerHTML = `<h3>${esc(group.name)}</h3><div class="badge">${group.members.length} / ${group.capacity}</div>`;
    const list = document.createElement('ul');
    list.innerHTML = group.members.map(m=>`<li>${esc(m.name)} <button type="button" onclick="removeMember('${group.id}','${m.id}')">Hapus</button></li>`).join('');
    card.appendChild(header);
    card.appendChild(list);
    container.appendChild(card);
  });
